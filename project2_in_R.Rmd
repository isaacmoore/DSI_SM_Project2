...
time_on_bb <- data2 %>% select(track, time_secs, rank) %>% 
        group_by(track) %>% 
                mutate(time_on_bbs = n_distinct(rank)) %>% 
                        arrange(desc(time_on_bbs))

ggplot(time_on_bb, aes(time_secs, time_on_bbs)) + geom_point() + geom_smooth() + ggtitle('Correlation between song length (seconds)\n and time (weeks) on the Billboard Top 100')
```
